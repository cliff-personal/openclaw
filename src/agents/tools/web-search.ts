...
async function proxyFetch(input: string | URL, init?: RequestInit): Promise<Response> {
  ...
  if (dispatcher) {
    // Node's global fetch is powered by undici and supports the non-standard
    // `dispatcher` option. Using global fetch keeps unit tests (which mock
    // globalThis.fetch) from accidentally hitting the network.
...
<<<<<<< HEAD
      : provider === "duckduckgo"
        ? "Search the web using DuckDuckGo (free; no API key required). Returns titles, URLs, and snippets for fast research."
=======
      : provider === "grok"
